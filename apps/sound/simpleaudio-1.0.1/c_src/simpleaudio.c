...
static PyObject* _play_buffer(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "Oiii|O", &audio_obj, &num_channels, &bytes_per_channel, &sample_rate, &options)) {
        return NULL;
    }
    ...
    /* fixed 100ms latency */
    return play_os(buffer_obj, num_samples, num_channels, bytes_per_channel, sample_rate, &play_list_head, SA_LATENCY_US, options);
}
