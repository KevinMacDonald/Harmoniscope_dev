...
PyObject* play_os(Py_buffer buffer_obj, int len_samples, int num_channels, int bytes_per_chan,
                  int sample_rate, play_item_t* play_list_head, int latency_us, PyObject* options) {
    ...
    if (options != NULL) {
        PyObject* device_obj = PyDict_GetItemString(options, "device");
        if (device_obj != NULL) {
            PyObject *device_str = PyObject_Str(device_obj);
            PyObject *device_str_bytes = PyUnicode_AsUTF8String(device_str); 

            free(device);
            device = strdup(PyBytes_AsString(device_str_bytes));

            Py_DECREF(device_str_bytes);
